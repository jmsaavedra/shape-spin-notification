...
/**
 * Calculate consecutive spin streak from spin history (current streak from most recent day backwards)
...
function calculateConsecutiveStreak(spins) {
  ...
  const now = new Date();
  const mostRecentSpinTime = Math.max(...spins.map(spin => Number(spin.timestamp) * 1000));
  ...
  // Calculate current consecutive streak from most recent day backwards
  let currentStreak = 1; // Start with 1 for the most recent spin day
  ...
  for (let i = 1; i < uniqueDates.length; i++) {
    ...
    if (dayDiff === 1) {
      ...
      // Gap found - stop counting, current streak is complete
      break;
    }
  }
