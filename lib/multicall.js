...
async function batchContractCalls(calls, provider) {
  try {
    ...
    console.log(`Multicall3 found at ${MULTICALL3_ADDRESS}, batching ${calls.length} calls`);

    const multicall = new ethers.Contract(MULTICALL3_ADDRESS, MULTICALL3_ABI, provider);
    
    // Prepare call data
    const call3s = calls.map((call, i) => {
      // In ethers v6, the address is stored as .target
      // Also try .address for compatibility
      const contractAddress = call.contract.target || call.contract.address || call.contract.getAddress?.();
      
      if (!contractAddress) {
        console.error(`Contract ${i} details:`, {
          hasTarget: !!call.contract.target,
          hasAddress: !!call.contract.address,
          method: call.method,
          contractKeys: Object.keys(call.contract)
        });
        throw new Error(`Contract address is null for method ${call.method}`);
      }
    ...
    const results = await multicall.aggregate3.staticCall(call3s);
