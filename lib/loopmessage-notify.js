...
class LoopMessageNotifier {
  ...
  async sendAlreadyCompletedNotification(spinNumber, nextSpinTime) {
    try {
      const message = {
        recipient: this.recipientNumber,
        text: `Shape Spin Notification app just checked and you've completed today's spin already. Nice! You can spin again on: ${nextSpinTime}`,
        sender_name: this.senderName
      };

      const response = await this.sendMessage(message);
      console.log('Already completed notification sent:', response);
      return response;
    } catch (error) {
      console.error('Failed to send already completed notification:', error);
      throw error;
    }
  }
}
